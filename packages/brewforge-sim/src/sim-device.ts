...
export class SimDevice {
  ...
  /**
   * Синхронная перемотка варочного времени на simSeconds вперёд — демо-прогрев:
   * пульт стартует «в разгаре затирания», а не с холодного котла. В отличие от
   * advanceToNow() не привязана к wall-clock и не ограничена MAX_CATCHUP_MS —
   * это явная просьба «прокрути столько-то», а не catch-up после простоя.
   * Промптов не подтверждает: на waitingAck конечный автомат сам стоит на месте,
   * лишние тики безвредны (вызывающий отвечает на промпт и продолжает перемотку).
   */
  warp(simSeconds: number): void {
    const perTickSec = (this.cfg.tickMs / 1000) * this.cfg.tickScale;
    if (perTickSec <= 0 || simSeconds <= 0) return;
    const ticks = Math.min(Math.ceil(simSeconds / perTickSec), 10_000);
