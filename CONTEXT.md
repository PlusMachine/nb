...
| **features/brew-batches/components/**, **features/devices/components/** | Варка: `brew-journal`, `brew-lifecycle`, `brew-notes`, `live-dashboard`, `telemetry-chart`. Устройства: `devices-manager`, `device-config-form`, `device-picker-list.tsx`, `brew-recipe-on-device-picker.tsx` (выбор устройства из `BrewPickerDialog`, флоу — `features/brew-controller/brew-recipe-flow.ts`) |
