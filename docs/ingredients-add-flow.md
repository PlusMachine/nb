...
- для `consumable` и `water_treatment` показывается поле `Подтип`
- для `fermentable` subtype уже задается верхней сеткой категорий `Солод / Сбраживаемое сырье`, поэтому дублирующий selector внутри custom form не показывается
