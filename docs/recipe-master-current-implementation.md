...
Секция `Другие добавки` используется для misc/process additions:

- техдобавки: фининги, ферменты, нутриенты, антиоксиданты, пеногасители, консерванты;
- фильтрация затора: рисовая/овсяная/ячменная лузга;
- специи;
- цедра и цитрус;
- травы и цветы;
- кофе/какао;
...
В add flow эта секция соответствует складской группе `Другие добавки` (`consumable` + `inventory_additives`) и новому seed `additives_v2_1.json`. Складские `Расходники` (`inventory_supplies`: мойка, санитайзеры, тара, газы) не показываются как варианты для добавления в рецепт.
...
5. `Расчет`
   - auto solver показывает соли/кислоту, но не добавляет их в список добавок сам;
   - кнопка `Применить расчет` создает фактический список добавок;
   - если план уже есть, кнопка становится `Заменить добавки`;
   - фактические добавки редактируются в секции водоподготовки выше.
6. `Расширенные настройки`
   ...
   - сода включается отдельным checkbox для расчетного предложения.
