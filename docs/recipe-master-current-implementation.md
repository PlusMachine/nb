...
4. `Что добавить`
   ...
   - `Целевой pH затора` находится здесь: под общим объемом воды или под заторной водой в split mode;
   - checkbox `Рассчитывать pH затора` переводит расчет в pH mode; если выключен, `targetMashPh = null` и считается только минерализация;
   - кислота показывается в relevant card.
5. `Расширенные настройки`
   - выбор `Авторасчет солей / Ручные добавки солей`;
   - модель pH, кислота, концентрация и калибровка показываются только если включен расчет pH затора;
   - manual salt additions с удалением строки;
   - редкие salts сгруппированы как advanced.

### Sparge acidification

Подкисление промывочной воды теперь живет внутри карточки `В промывку`, а не отдельным глобальным блоком.
...
- total mineralization volume from recipe batch size;
