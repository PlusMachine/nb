...
- `apps/web/components/recipes/recipe-water-additives-section.tsx`;
...
Вода рендерится в `RecipeDesigner` как секция ингредиентов `Водоподготовка`:

- после `Дрожжи`;
- перед `Другие добавки`;
- внутри того же основного flow рецепта, где пользователь работает с составом.

Отдельного wizard-screen только для воды нет: настройка, расчетное предложение и фактические добавки живут в одной секции.

## 2. Верхний UX блока: как пользователь в него входит

Внешний контейнер воды — это обычный ingredient section `Водоподготовка`.

В header секции видны:

- заголовок `Водоподготовка`;
- счетчик фактических добавок, если они есть;
- кнопка `Настроить воду` / `Скрыть настройку`;
- кнопка `Сбросить воду`, если `waterPlanMeta.setupEnabled === true`.

Внутри секции порядок такой:

1. `RecipeWaterAdditivesSection` — фактические добавки воды.
2. Legacy/manual `recipe_ingredients` категории `water_treatment`, если они уже есть в рецепте.
3. Embedded `WaterSetupWizard` — настройка source/target/объемов/pH и расчетное предложение.

`RecipeWaterAdditivesSection` не показывает расчетные соли сам по себе. Пока пользователь не нажал `Применить расчет` или не добавил соль вручную, блок остается в empty state `Нет добавок воды`.

Важно:

- расчетное предложение не смешивается с фактическими добавками;
- отдельного вопроса `Настроить водоподготовку?` нет;
- настройка воды и список добавок разделены визуально.

Пользователь входит в настройку воды через header action или раскрывая внутренний блок `Настройка воды`.
...
- заголовок секции `Водоподготовка`;
- блок `Добавки воды`;
- empty state `Нет добавок воды`;
...
Он не видит шаги `3. Объем воды`, `4. pH и подкисление`, `Расчет` и `Расширенные настройки`, пока вода не будет активирована первым meaningful action.

Отдельного шага `Что добавить` в настройках нет: расчет живет в блоке `Расчет`, а фактические соли и кислота живут выше в `Добавки воды`.
...
```text
Увидеть блок "Водоподготовка" в основном flow рецепта
-> увидеть фактические "Добавки воды"
...
-> посмотреть рассчитанные соли / кислоту в блоке "Расчет"
-> нажать "Применить расчет", чтобы перенести расчет в "Добавки воды"
-> при необходимости скорректировать граммы или место внесения в "Добавки воды"
-> при необходимости открыть advanced и сменить pH model, acid, concentration, calibration
...
Это и есть фактическая граница между "source/target only" и "full water plan". Расчет не становится частью рецепта сам: он попадает в `Добавки воды` только после явного действия пользователя.

## 9. RecipeWaterAdditivesSection: что реально показывается

`RecipeWaterAdditivesSection` рендерится вверху секции `Водоподготовка`.

Если `waterPlanMeta.setupEnabled !== true`, секция показывает empty state:

```text
Нет добавок воды
```

Если расчет уже применен или соль добавлена вручную, секция показывает:
...
В split mode строки группируются по `Весь объем`, `Затор`, `Промывка`.

Правила:

- расчетные соли не показываются в этой секции до `Применить расчет`;
- итоговый профиль воды показывается только когда есть реальные строки добавок;
- ручные соли редактируются прямо в строке;
- кислота не редактируется как отдельная строка, ее параметры живут в настройках pH/advanced.
...
- split mode: suggested split или текущий total volume в литрах.
...
Отдельного шага `5. Что добавить` больше нет. Расчетные соли и кислота показываются в блоке `Расчет` внутри раскрытой настройки воды.

### 13.1. One-volume mode

В one-volume mode блок `Расчет` показывает одну группу:

- `Весь объем`;
- список рассчитанных солей;
- строку кислоты, если включен mash pH и расчет дал положительный объем.

Кнопка рядом с заголовком:

- `Применить расчет`, если фактических добавок еще нет;
- `Заменить добавки`, если в верхней секции уже есть строки.

### 13.2. Split mode

В split mode блок `Расчет` показывает группы:

- `Весь объем`;
- `Затор`;
- `Промывка`.

В `Затор` попадают `mashSaltAdditions` и mash acid. В `Промывка` попадают `spargeSaltAdditions` и sparge acid.
...
Если calculated additions нет, карточка пишет:
...
Acid row попадает в `Расчет` и затем в `Добавки воды` только если расчет дал положительный объем кислоты.
...
Warnings показываются над раскрываемой настройкой воды, только первые 3 visible warnings:
...
## 14. Расчет и добавки воды

UI больше не показывает пользователю отдельный режим `Авторасчет / Ручной план`.

Модель теперь такая:

- расчет в мастере воды — это preview, а не часть рецепта;
- верхняя секция `Водоподготовка` показывает только добавки воды;
- кнопка `Применить расчет` копирует текущие рассчитанные соли в `manualSaltAdditions`;
- если добавки уже есть, кнопка называется `Заменить добавки`;
- после применения строки солей редактируются прямо в верхней секции.

Технически фактические соли по-прежнему хранятся через `engine = "advanced_manual"` и `manualSaltAdditions`, но это не пользовательский режим.

### 14.1. Расчет

Блок `Расчет` живет внутри раскрытой настройки воды после шага `pH и подкисление`.

Он показывает:

- рассчитанные соли;
- рассчитанную кислоту, если включена коррекция pH затора или промывки;
- группировку по `Весь объем`, `Затор`, `Промывка` при split-volume.

До нажатия `Применить расчет` эти строки не появляются в верхнем списке добавок.

### 14.2. Добавки воды

`RecipeWaterAdditivesSection` показывает:

- title `Добавки воды`;
- фактические строки солей из `manualSaltAdditions`;
- рассчитанную кислоту только рядом с фактическими добавками;
- итоговый профиль воды только когда есть реальные строки.

Если расчет не применен, calculated auto salts не рендерятся в верхней секции.
...
### 14.5. `Считать пищевую соду (NaHCO3) в авторасчете`

Checkbox visible в advanced settings, когда расчетное предложение строится auto solver-ом.
...
- empty state `Нет добавок воды` — рендерится в `RecipeWaterAdditivesSection`, пока нет фактических добавок;
...
20. В split mode изменить:
   - `Исходный pH`;
   - `Целевой pH промывки`.
21. Проверить блок `Расчет`.
22. Нажать `Применить расчет` или `Заменить добавки`.
23. В верхнем списке добавок:
   - изменить grams соли;
   - выбрать цель внесения в split mode;
   - удалить строку соли.
24. Открыть `Расширенные настройки`.
25. При включенном mash pH изменить:
   - `Модель pH`;
   - `Калибровка pH`.
26. При включенном подкислении затора или промывки изменить:
   - `Кислота`;
   - `Концентрация кислоты, %`.
