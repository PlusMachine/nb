...
- для этого предусмотрен отдельный flow `обнулить остаток`.
...
1. У позиции с количеством `> 0` показывается action `обнулить остаток`.
...
- кнопка `обнулить остаток` вызывает `updateInventoryInlineAction(...)`;
...
- это inline action `обнулить остаток`;
