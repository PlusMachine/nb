...
Список категорий в UI: `Солод`, `Сбраживаемое сырье`, `Хмель`, `Дрожжи`, `Водоподготовка`, `Расходники`, `Специи и добавки`.
...
- Верхний split `Расходники` / `Специи и добавки` покрывает весь `consumable` каталог; group-chips
...
- **Неравномерная семантика chips.** Верхний ряд смешивает уровни taxonomy: `Сбраживаемые`/`Хмель`/`Дрожжи`/
  `Водоподготовка` — категории, а `Расходники`/`Специи и добавки` — broad groups внутри `consumable`; `Солод` —
...
(`ingredient-picker.tsx:1309`) возвращает `Уточнить группу водоподготовки` для `water_treatment`,
`Уточнить группу сбраживаемых` для `fermentable`, а для `consumable_group` смотрит на `broadGroup`:
`Уточнить группу добавок` при `inventory_additives`, `Уточнить группу расходников` при `inventory_supplies`,
иначе (нет форсированной группы, например пикер открыт из общего каталога) — `Уточнить группу добавок и
расходников`. Старое утверждение, что для water_treatment всё ещё показывается «Уточнить группу
