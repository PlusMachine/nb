...
В `RecipeDesigner` секция рендерится среди ингредиентов (после `Дрожжи`, перед `Специи и добавки`). Порядок внутри секции:
...
`Расширенные настройки` (`<details>`, свёрнут): при `mashPhEnabled` — `Модель pH` (`Быстрая по RA (Кольбах)` / `По составу засыпи (Riffe)`) и `Калибровка pH` (range `-2..2`, step `0.01`, placeholder `0.00`); при включённом любом acid calc — `Кислота` (`Молочная` / `Фосфорная`) и `Концентрация кислоты, %` (optional, placeholder `88`/`85`); checkbox `Считать пищевую соду (NaHCO3) в авторасчёте` (default off).
