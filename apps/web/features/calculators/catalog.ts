...
export const calculators: CalculatorCatalogItem[] = [
  ...
  },
  {
    ...
    meaning: ["Если объём растёт, плотность падает; если объём уходит в испарение, плотность растёт."]
  },
  {
    slug: "ibu",
  ...
  },
  {
    ...
    intro: "Быстрый расчёт воды по цели в ферментере, зерну, испарению и потерям оборудования.",
    ...
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Всего воды = объём до кипячения + впитывание зерном + потери в котле. Объём до кипячения = горячий объём после кипячения + испарение, а горячий объём после кипячения считается от объёма в ферментер и потерь на осадок/чиллер с поправкой на усадку при охлаждении.",
    ...
    nextSteps: ["Перенесите объёмы затора и промывки в калькулятор воды и pH.", "Уточните свое испарение по реальным варкам."],
    meaning: ["Расчёт показывает, где теряется объём и сколько воды нужно подготовить заранее."]
  },
  {
    slug: "mash-infusion",
    title: "Инфузионное затирание (доливом кипятка)",
    seoTitle: "Калькулятор температуры заторной воды",
    shortTitle: "Инфузионное затирание",
    description: "Температура заторной воды на старте и объём кипятка на следующую ступень.",
    seoDescription: "Считает температуру заторной воды по гидромодулю и целевой температуре затора, а также объём кипятка для инфузионного (доливом) перехода на следующую ступень — по тепловому балансу зерна и воды.",
    intro: "Перед засыпью — какой температуры взять воду на старт; на ступенчатом затирании доливом — сколько кипятка подмешать для следующей паузы.",
    whenToUse: "Перед засыпью зерна в воду или при переходе на следующую температурную ступень методом долива кипятка.",
    section: "Варка и рецепт",
    groups: ["Варка", "Вода"],
    aliases: ["заторная вода", "затирание", "strike", "инфузия", "паузы", "мэш-аут", "температура затора", "гидромодуль", "mash", "инфузионное затирание", "настойное затирание", "долив кипятка"],
    ...
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Температура заторной воды считается из теплового баланса: вода отдаёт тепло зерну (и, если задано, холодному котлу), пока всё не выровняется на целевой температуре затора. Теплоёмкость зерна — 0,41 л·°C/кг воды. Долив кипятка на следующую ступень считается тем же балансом в обратную сторону: сколько кипятка нужно подмешать в уже тёплый затор, чтобы поднять его до новой температуры.",
    assumptions: [...sharedAssumptions, "Расчёт не учитывает теплопотери в воздух во время самого затирания — только тепловой баланс воды, зерна и (опционально) котла в момент смешивания."],
    commonMistakes: ["Холодный котёл забирает несколько градусов у заторной воды — прогрейте его заранее или укажите термомассу.", "Температуру затора мерят до того, как засыпь размешана и прогрелась равномерно — показание получается неточным."],
    ...
    meaning: ["Чем гуще затор (меньше воды на кг зерна), тем горячее нужна заторная вода на ту же целевую температуру.", "Долив кипятка на следующую ступень — тот же расчёт в обратную сторону: сколько кипятка подмешать, чтобы поднять уже тёплый затор до нужной температуры."]
  },
  {
    slug: "water-ph",
    ...
    commonMistakes: ["Вводят щелочность (CaCO3) вместо гидрокарбоната HCO3.", "Не разделяют объём заторной и промывочной воды."],
  ...
  },
  {
    ...
    intro: "Считает остаточный CO2 по температуре и дозу сахара на весь объём или на бутылку.",
    whenToUse: "Перед розливом в бутылки, когда известны объём пива, температура и целевой CO2.",
    ...
    nextSteps: ["Передайте объём в калькулятор «Бутылки и розлив».", "Сравните с шпайзе или кройценом, если хотите натуральную карбонизацию суслом."],
  ...
  },
  {
    ...
    formula: "Бутылок = целое от ((объём пива − потери при розливе) / объём бутылки).",
    assumptions: sharedAssumptions,
    commonMistakes: [],
  ...
  },
  {
    ...
    description: "Объём сусла или кройцена для натуральной карбонизации.",
    ...
    related: [{ href: "/bjcp", label: "Стили пива" }],
    formula: "Нужный CO₂ переводим в массу: 1 об. CO₂ = 1,96 г/л. Килограмм сброженного экстракта даёт ~463 г CO₂; реально сбраживается ~63% экстракта шпайзе (у кройцена меньше — часть уже выброжена). Отсюда объём шпайзе по его плотности.",
    assumptions: [...sharedAssumptions, "Расчёт приблизительный: фактическая сбраживаемость шпайзе и кройцена отличается от расчётной."],
    ...
    meaning: ["Результат показывает объём сусла, который должен дать недостающий CO2."]
  },
  {
    slug: "beer-color",
  ...
  },
  {
    ...
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Пункты плотности = вес зерна (в фунтах) × экстрактивность засыпи (PPG) × эффективность / объём партии (в галлонах). Сахар, сухой и жидкий экстракт растворяются полностью и не зависят от эффективности варки — их пункты считаются отдельно от засыпи зерна.",
    assumptions: [...sharedAssumptions, "Экстрактивность (PPG) подставляется из каталога ингредиентов при выборе солода — можно поправить по цифре с упаковки."],
    commonMistakes: ["Путают эффективность затора (только до фильтрации) и эффективность всей варки (с учётом потерь при кипячении и переливах) — это разные числа.", "Меряют НП до того, как доливная вода до целевого объёма полностью размешалась с суслом."],
