...
export const calculators: CalculatorCatalogItem[] = [
  ...
  },
  {
    ...
    assumptions: [...sharedAssumptions, "Расчет не учитывает теплопотери в воздух во время самого затирания — только тепловой баланс воды, зерна и (опционально) котла в момент смешивания."],
    commonMistakes: ["Холодный котел забирает несколько градусов у заторной воды — прогрейте его заранее или укажите термомассу.", "Температуру затора мерят до того, как засыпь размешана и прогрелась равномерно — показание получается неточным."],
    nextSteps: ["Перенесите объем воды в калькулятор воды на варку.", "Проверьте профиль воды и pH после того, как определитесь с объемами."],
    meaning: ["Чем гуще затор (меньше воды на кг зерна), тем горячее нужна заторная вода на ту же целевую температуру.", "Долив кипятка на следующую ступень — тот же расчет в обратную сторону: сколько кипятка подмешать, чтобы поднять уже теплый затор до нужной температуры."]
  },
  {
    slug: "water-ph",
    ...
    intro: "Легкий расчет профиля воды и pH затора без обязательного рецепта — вручную или подбором солей под классический профиль стиля.",
    ...
    assumptions: [...sharedAssumptions, "Расчет pH затора — ориентировочная модель, не замена pH-метру.", "Подбор солей не умеет снижать ионы — только добавлять; если в исходной воде уже избыток щелочности или иона сверх цели, эту разницу убирает кислота, а не соли."],
  ...
  },
  {
    ...
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Пункты плотности = вес зерна (в фунтах) × экстрактивность засыпи (PPG) × эффективность / объем партии (в галлонах). Сахар, сухой и жидкий экстракт растворяются полностью и не зависят от эффективности варки — их пункты считаются отдельно от засыпи зерна.",
    ...
    commonMistakes: ["Путают эффективность затора (только до фильтрации) и эффективность всей варки (с учетом потерь при кипячении и переливах) — это разные числа.", "Меряют НП до того, как доливная вода до целевого объема полностью размешалась с суслом."],
    nextSteps: ["Передайте свою эффективность в расчет воды на варку для следующей партии.", "Сравните итоговую крепость и сбраживание в калькуляторе ABV."],
