...
export const calculators: CalculatorCatalogItem[] = [
  ...
  },
  {
    ...
    intro: "Обычный Brix-to-SG после старта брожения ошибается, потому что спирт меняет преломление.",
    ...
    formula: "До брожения: correctedBrix = rawBrix / WCF. После брожения: алкогольная коррекция Novotny или Terrill.",
    assumptions: [...sharedAssumptions, "WCF по умолчанию 1.04, но лучше откалибровать под свой рефрактометр."],
    commonMistakes: ["Используют обычный Brix -> SG после начала брожения.", "Не вводят исходную плотность для алкогольной коррекции."],
