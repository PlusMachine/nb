...
export const calculators: CalculatorCatalogItem[] = [
  {
    ...
    relatedSlugs: ["hydrometer-correction", "refractometer-correction", "priming-sugar", "keg-carbonation", "brewhouse-efficiency"],
    ...
    meaning: ["ABV показывает объёмную долю спирта.", "Видимое сбраживание помогает понять, насколько глубоко дрожжи переработали сусло."]
  },
  {
    ...
    assumptions: [...sharedAssumptions, "WCF по умолчанию 1.04; точнее — откалибровать под свой прибор встроенным калькулятором WCF (один раз по суслу до брожения).", "OG в Brix — сырое показание рефрактометра, делится на WCF. OG в SG или °P — истинная плотность с ареометра/сахаромера или из рецепта, WCF не применяется."],
    commonMistakes: [],
  ...
  },
  {
    ...
    title: "Разбавить или уварить сусло",
    seoTitle: "Калькулятор разбавления и уваривания сусла",
    shortTitle: "Разбавление сусла",
    ...
    commonMistakes: ["Сравнивают горячий объём с холодным без поправки на усадку при охлаждении.", "Используют показание ареометра без поправки на температуру."],
    nextSteps: ["Передайте новый объём в IBU.", "Проверьте расчёт воды на следующую варку."],
    meaning: ["Если объём растёт, плотность падает; если объём уходит на выкипание, плотность растёт."]
  },
  {
    slug: "ibu",
    ...
    description: "Расчёт горечи по хмелю, времени внесения и объёму.",
    ...
    meaning: ["IBU показывает расчётную горечь, а BU:GU помогает понять баланс относительно плотности."]
  },
  {
    ...
    description: "Общий объём, затор, промывка, объёмы до и после кипячения.",
    seoDescription: "Рассчитывает общий объём воды на варку: затор, промывку, выкипание при кипячении и потери оборудования — от целевого объёма в ферментере до объёма перед варочным котлом.",
    intro: "Быстрый расчёт воды по цели в ферментере, зерну, выкипанию и потерям оборудования.",
    whenToUse: "Перед затиранием, когда нужно понять общий объём воды и разделение на затор/промывку.",
    section: "Варка и рецепт",
    groups: ["Варка", "Вода"],
    aliases: ["вода на варку", "mash water", "sparge", "pre boil", "biab", "объём воды"],
    stageChips: ["Вода", "Варка"],
    accentClassName: "border-l-cyan-400",
    href: "/calculators/brewing-water-volume",
    relatedSlugs: ["water-ph", "dilution-boiloff", "ibu", "unit-converter", "mash-infusion"],
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Всего воды = объём до кипячения + впитывание зерном + потери в котле. Объём до кипячения = горячий объём после кипячения + выкипание, а горячий объём после кипячения считается от объёма в ферментер и потерь на осадок/чиллер с поправкой на усадку при охлаждении.",
    assumptions: [...sharedAssumptions, "Усадка при охлаждении по умолчанию 4%."],
    commonMistakes: ["Оставляют нулевые потери на осадок и в котле — расчёт сойдется, но пива в ферментере окажется меньше.", "Путают объём в ферментер и объём после кипячения."],
    nextSteps: ["Перенесите объёмы затора и промывки в калькулятор воды и pH.", "Уточните своё выкипание по реальным варкам."],
    meaning: ["Расчёт показывает, где теряется объём и сколько воды нужно подготовить заранее."]
  },
  {
    slug: "mash-infusion",
    title: "Инфузионное затирание",
    seoTitle: "Калькулятор температуры заторной воды",
    shortTitle: "Инфузионное затирание",
    description: "Температура заторной воды на старте, долив на следующую паузу (кипятком или холодной водой) и лестница ступенчатого затора целиком.",
    seoDescription: "Считает температуру заторной воды по гидромодулю и целевой температуре, долив на следующую ступень — кипятком вверх или холодной водой вниз — и лестницу пауз ступенчатого затора по тепловому балансу зерна и воды.",
    intro: "Перед засыпью — какой температуры взять воду на старт; дальше — сколько долить на следующую паузу (кипятка, а перегрели — холодной воды) или сразу вся лестница пауз ступенчатого затора.",
    whenToUse: "Перед засыпью зерна в воду, при переходе на следующую температурную ступень доливом — включая обратную задачу «перегрел затор — сколько долить холодной» — и при планировании ступенчатого затора целиком.",
    section: "Варка и рецепт",
    groups: ["Варка", "Вода"],
    aliases: ["заторная вода", "затирание", "strike", "инфузия", "паузы", "мэш-аут", "температура затора", "гидромодуль", "mash", "инфузионное затирание", "настойное затирание", "долив кипятка", "ступенчатое затирание", "ступенчатый затор", "step mash", "перегрел затор", "остудить затор"],
    stageChips: ["Затор", "Температура"],
    accentClassName: "border-l-teal-400",
    href: "/calculators/mash-infusion",
    relatedSlugs: ["brewing-water-volume", "water-ph", "brewhouse-efficiency", "unit-converter"],
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Температура заторной воды считается из теплового баланса: вода отдаёт тепло зерну (и, если задано, холодному заторнику), пока всё не выровняется на целевой температуре затора. Теплоёмкость зерна — 0,41 л·°C/кг воды. Долив на следующую ступень считается тем же балансом в обратную сторону: сколько воды нужно подмешать в уже тёплый затор, чтобы сдвинуть его к новой температуре. В режиме «Долив на ступень» направление определяет само уравнение: пауза выше текущей — долив кипятка поднимает температуру, пауза ниже — долив холодной воды её опускает. «Ступенчатый затор» повторяет расчёт по всей лестнице пауз подряд, всегда доливом кипятка: каждый следующий долив считается уже от выросшего объёма воды в заторе, а не от стартового, а пауза ниже предыдущей считается остыванием затора без долива.",
    assumptions: [...sharedAssumptions, "Расчёт не учитывает теплопотери в воздух во время самого затирания — только тепловой баланс воды, зерна и (опционально) заторника в момент смешивания."],
    commonMistakes: ["Холодный заторник забирает несколько градусов у заторной воды — прогрейте его заранее или укажите теплоёмкость.", "Температуру затора мерят до того, как засыпь размешана и прогрелась равномерно — показание получается неточным.", "На лестнице из нескольких пауз забывают, что каждый долив увеличивает объём воды в заторе — следующая ступень считается уже от него, а не от стартового объёма.", "Пробуют поднять температуру доливом холодной воды или опустить доливом кипятка — по факту это разные расчёты (кипяток поднимает, холодная вода опускает). В режиме «Долив на ступень» калькулятор сам определяет нужный по направлению паузы; в лестнице «Ступенчатого затора» доливы всегда кипятком, а нисходящая пауза — остывание без долива."],
    nextSteps: ["Перенесите объём воды в калькулятор воды на варку.", "Проверьте профиль воды и pH после того, как определитесь с объёмами."],
    meaning: ["Чем гуще затор (меньше воды на кг зерна), тем горячее нужна заторная вода на ту же целевую температуру.", "Долив на следующую ступень — тот же расчёт в обратную сторону: сколько воды подмешать, чтобы сдвинуть уже тёплый затор к нужной температуре, будь то нагрев кипятком или охлаждение холодной водой.", "На ступенчатом заторе объём воды нарастает от паузы к паузе — итоговый гидромодуль в мэш-ауте обычно заметно жиже, чем на старте затирания."]
  },
  {
    slug: "water-ph",
    ...
    description: "Ионы, соли, кислота, соотношение SO₄:Cl и ориентировочный pH затора.",
    seoDescription: "Считает итоговый профиль воды после добавления солей и кислоты, соотношение сульфат:хлорид и ориентировочный pH затора — вручную или подбором солей под целевой профиль (IPA, лагер, янтарное, тёмное).",
    intro: "Лёгкий расчёт профиля воды и pH затора без обязательного рецепта — вручную или подбором солей под классический профиль стиля.",
    whenToUse: "Когда нужно быстро оценить соли, кислоту и pH затора перед варкой, или подобрать соли под целевой профиль воды, зная только свою исходную воду.",
    section: "Варка и рецепт",
    groups: ["Вода", "Варка"],
    ...
    relatedSlugs: ["brewing-water-volume", "unit-converter", "beer-color", "mash-infusion"],
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Щелочность (как CaCO₃) = HCO₃ × 50 / 61; итоговый профиль воды складывается из добавок солей в ppm. В режиме «Подобрать соли» перебором ищется доза гипса, хлористого кальция, английской соли, поваренной соли и соды, которая ближе всего подводит вашу исходную воду к выбранному целевому профилю.",
    assumptions: [...sharedAssumptions, "Расчёт pH затора — ориентировочная модель, не замена pH-метру.", "Подбор солей не умеет снижать ионы — только добавлять; если в исходной воде уже избыток щелочности или иона сверх цели, эту разницу убирает кислота, а не соли."],
    commonMistakes: ["Вводят щелочность (CaCO₃) вместо гидрокарбоната HCO₃.", "Не разделяют объём заторной и промывочной воды."],
    nextSteps: ["Перенесите расчётные объёмы из калькулятора воды на варку.", "Проверьте цветность, если pH выглядит слишком высоким или низким."],
  ...
  },
  {
    ...
    description: "Норма засева, живые клетки, недобор/избыток и объём стартера.",
    ...
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Нужно клеток = объём (мл) × Plato × норма засева, где эль = 0,75, лагер = 1,5, гибрид = 1,0 млн клеток/мл/°P.",
    ...
    nextSteps: ["Плотность и пропорция сусла стартера — в результате расчёта; остудите его до температуры засева перед внесением.", "Если известен только SG, переведите его в Plato через конвертер единиц."],
  ...
  },
  {
    ...
    description: "Сколько декстрозы, обычного сахара, сухого солодового экстракта или мёда нужно на объём и бутылку.",
    seoDescription: "Считает дозу сахара, декстрозы, сухого солодового экстракта или мёда на карбонизацию — по остаточному CO₂ при температуре пива в конце брожения и целевому объёму газа в бутылке.",
    intro: "Считает остаточный CO₂ по температуре и дозу сахара на весь объём или на бутылку.",
    whenToUse: "Перед розливом в бутылки, когда известны объём пива, температура и целевой CO₂.",
    section: "Розлив и карбонизация",
    groups: ["Розлив"],
    aliases: ["прайминг", "праймер", "карбонизация", "сахар", "декстроза", "бутылки", "co2"],
    stageChips: ["Розлив", "CO₂"],
    ...
    assumptions: [...sharedAssumptions, "Остаточный CO₂ считается по температуре пива в конце брожения: CO₂, вышедший из пива при этой температуре, после охлаждения обратно уже не растворяется."],
    ...
    nextSteps: ["Передайте объём в калькулятор «Бутылки и розлив».", "Сравните с шпайзе или краузеном, если хотите натуральную карбонизацию суслом."],
    meaning: ["Чем теплее пиво было после брожения, тем меньше остаточного CO₂ и тем больше сахара нужно."]
  },
  {
    ...
    description: "Давление (бар/PSI) для целевого CO₂ при заданной температуре.",
    seoDescription: "Таблица карбонизации «температура × давление»: показывает, какое давление выставить на кеге или шпунт-клапане для целевого объёма CO₂ при заданной температуре пива.",
    intro: "Классическая таблица карбонизации «температура × давление»: выберите стиль или введите целевой CO₂ — калькулятор покажет, какое давление ставить на кеге или шпунт-клапане.",
    ...
    commonMistakes: [
      "Снимать пробу раньше времени. При выставленном давлении и постоянной температуре расчётная карбонизация набирается примерно за 7–14 дней. Быстрее — только форсированно: поднять давление выше расчётного и покатать или потрясти кег, но так легко перегазовать пиво.",
      "Подставлять в расчёт температуру холодильника, а не пива. Кег остывает медленно — полной партии нужны примерно сутки, — а тёплое пиво при том же давлении насыщается слабее.",
      "Сбрасывать давление до минимума, чтобы победить пену при розливе. За несколько дней на заниженном давлении пиво теряет карбонизацию. Пену лечат балансировкой линии — длиной и диаметром шланга, — а не сбросом давления."
    ],
    nextSteps: ["Переведите давление в нужные единицы.", "Сравните с сахарной карбонизацией для бутылочной партии."],
    meaning: ["CO₂ лучше растворяется в холодном пиве: чем ниже температура, тем меньшее давление нужно для той же карбонизации."]
  },
  {
    slug: "bottling",
    title: "Бутылки и объём розлива",
    seoTitle: "Калькулятор бутылок и объёма розлива",
    shortTitle: "Бутылки и розлив",
    description: "Сколько бутылок нужно, остаток объёма и сахар на бутылку.",
    ...
    formula: "Бутылок = целое от ((объём пива − потери при розливе) / объём бутылки). Со второй тарой: сначала целыми бутылками основного размера, остаток — целыми бутылками второго.",
    assumptions: sharedAssumptions,
    commonMistakes: ["При смешанной таре считают сахар для прайминга одной дозой на все бутылки — а расход сахара пропорционален объёму бутылки, у 0.5 л и 0.33 л он разный."],
    nextSteps: ["Вернитесь в карбонизацию сахаром, если поменялся объём розлива.", "Подготовьте несколько запасных бутылок меньшего размера."],
    meaning: ["Остаток — сколько пива не влезет в целые бутылки: при одной таре — выбранного размера, при смешанной — после обеих."]
  },
  {
    slug: "speise-krausen",
    title: "Карбонизация суслом (шпайзе, краузен)",
    seoTitle: "Калькулятор карбонизации шпайзе и краузеном",
    shortTitle: "Шпайзе и краузен",
    description: "Объём сусла или краузена для натуральной карбонизации.",
    seoDescription: "Считает объём сусла (шпайзе) или активно бродящего пива (краузена), который даёт нужную карбонизацию без сахара — по целевому CO₂ и плотности добавляемого сусла.",
    intro: "Альтернатива карбонизации сахаром, когда газируют суслом или активно бродящим пивом.",
    whenToUse: "Перед розливом, если газируете шпайзе, краузеном или гайлом вместо сахара.",
    section: "Розлив и карбонизация",
    groups: ["Розлив", "Брожение"],
    aliases: ["шпайзе", "speise", "krausen", "краузен", "краузенинг", "кройцен", "gyle", "карбонизация"],
    stageChips: ["Розлив", "Натуральная CO₂"],
    accentClassName: "border-l-amber-400",
    href: "/calculators/speise-krausen",
    relatedSlugs: ["priming-sugar", "bottling", "keg-carbonation", "unit-converter"],
    related: [{ href: "/bjcp", label: "Стили пива" }],
    formula: "Нужный CO₂ переводим в массу: 1 об. CO₂ = 1,96 г/л. Килограмм сброженного экстракта даёт ~463 г CO₂; реально сбраживается ~63% экстракта шпайзе (у краузена меньше — часть уже выброжена). Отсюда объём шпайзе по его плотности.",
    assumptions: [...sharedAssumptions, "Расчёт приблизительный: фактическая сбраживаемость шпайзе и краузена отличается от расчётной."],
    ...
    meaning: ["Результат показывает объём сусла, который должен дать недостающий CO₂."]
  },
  {
    slug: "beer-color",
    ...
    description: "Расчёт цветности по засыпи и объёму партии.",
    ...
    intro: "Быстрая оценка цвета по объёму партии и списку солодов и экстрактов.",
    ...
    related: [{ href: "/bjcp", label: "Стили пива" }],
    formula: "MCU = сумма(вес в фунтах × цвет в Lovibond) / объём в галлонах; SRM = 1,4922 × MCU^0,6859; EBC = SRM × 1,97.",
    assumptions: sharedAssumptions,
  ...
  {
    slug: "brewhouse-efficiency",
    title: "Эффективность варки",
    seoTitle: "Калькулятор эффективности варки",
    shortTitle: "Эффективность",
    description: "Фактическая эффективность по замеру НП (начальная плотность) или прогноз НП по засыпи.",
    seoDescription: "Считает фактическую эффективность варки по замеренной НП и засыпи солодов и экстрактов, а также обратную задачу — прогнозирует НП партии по засыпи и заданной эффективности, в пунктах плотности на кг зерна.",
    intro: "После замера начальной плотности посчитайте фактическую эффективность варки или, наоборот, спрогнозируйте НП по засыпи и ожидаемой эффективности.",
    whenToUse: "После затирания и кипячения, когда есть замеренная НП, или заранее — чтобы прикинуть НП по засыпи и своей типичной эффективности.",
    section: "Измерения и плотность",
    groups: ["Варка", "Измерения"],
    aliases: ["эффективность", "efficiency", "нп по засыпи", "ppg", "экстрактивность", "выход", "brewhouse", "нп", "начальная плотность"],
    stageChips: ["Засыпь", "НП"],
    accentClassName: "border-l-indigo-400",
    href: "/calculators/brewhouse-efficiency",
    relatedSlugs: ["abv-attenuation", "dilution-boiloff", "mash-infusion", "beer-color"],
    related: [{ href: "/recipes", label: "Рецепты сообщества" }],
    formula: "Пункты плотности = вес зерна (в фунтах) × потенциал засыпи (PPG) × эффективность / объём партии (в галлонах). Сахар, сухой и жидкий экстракт растворяются полностью и не зависят от эффективности варки — их пункты считаются отдельно от засыпи зерна.",
    assumptions: [...sharedAssumptions, "Потенциал (PPG) подставляется из каталога ингредиентов при выборе солода — можно поправить по цифре с упаковки."],
    commonMistakes: ["Путают эффективность затора (только до фильтрации) и эффективность всей варки (с учётом потерь при кипячении и переливах) — это разные числа.", "Меряют НП до того, как доливная вода до целевого объёма полностью размешалась с суслом.", "Вписывают в PPG экстрактивность в процентах с упаковки солода (60–82% на СВ) — это другая шкала: PPG для базового солода обычно ~34–38, для сахара ~46."],
    nextSteps: ["Передайте свою эффективность в расчёт воды на варку для следующей партии.", "Сравните итоговую крепость и сбраживание в калькуляторе ABV."],
    meaning: ["65–80% — обычный диапазон эффективности для домашней варки.", "Стабильность важнее абсолютного числа: считайте рецепты по своей типичной эффективности, а не по книжной."]
  },
  {
    slug: "hop-freshness",
    ...
    seoDescription: "Оценивает текущую альфа-кислоту хмеля и потерю AA% при хранении по HSI-подобному коэффициенту, возрасту, температуре и типу упаковки — чтобы не завысить расчёт IBU старым или вскрытым хмелем.",
    intro: "Помогает не завысить расчёт IBU старым или вскрытым хмелем.",
    ...
    formula: "Потеря альфа-кислот моделируется по HSI-подобному коэффициенту, возрасту, температуре хранения, типу упаковки и форме хмеля.",
    assumptions: [...sharedAssumptions, "HSI здесь — коэффициент по мотивам индекса свежести хмеля (Hop Storage Index), а не спектрофотометрический показатель A275/A325 из лаборатории.", "Модель свежести приблизительная и не заменяет лабораторный анализ AA%."],
    ...
    nextSteps: ["Передайте расчётный AA% в IBU."],
  ...
  },
  {
    ...
    description: "Плотность, цвет, объём, вес, температура, давление и концентрации.",
...
// Облегчённая проекция для клиентских CalculatorsSearch/CalculatorCard: полный
// CalculatorCatalogItem тащит formula/assumptions/commonMistakes/nextSteps/meaning/
// seoDescription — длинные тексты, не нужные ни поиску, ни карточке, но раньше целиком
// улетавшие в RSC-payload клиентского дерева. Список секций (calculatorSections) клиент
// импортирует сам, а не получает пропом — он статичен и не зависит от набора карточек.
export type CalculatorCardItem = Pick<
  CalculatorCatalogItem,
  "slug" | "href" | "title" | "shortTitle" | "description" | "aliases" | "section" | "accentClassName"
>;

const toCalculatorCardItem = (item: CalculatorCatalogItem): CalculatorCardItem => ({
  slug: item.slug,
  href: item.href,
  title: item.title,
...
export const calculatorCardItems: CalculatorCardItem[] = calculators.map(toCalculatorCardItem);
