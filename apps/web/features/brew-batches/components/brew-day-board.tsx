...
function CurrentStepHero({
  ...
  if (!step) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50/60 p-5">
        ...
        <p className="mt-1 text-sm text-emerald-700">Перелейте сусло в ферментер, внесите дрожжи и поставьте на брожение.</p>
        ...
        {cursor.actComplete && !hasOg ? (
          <a href="#brew-journal" className="mt-2 inline-block text-xs text-amber-700 underline-offset-2 hover:underline">
  ...
  return (
    <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
      <p className="text-[11px] font-semibold uppercase tracking-wider text-zinc-400">Сейчас</p>
      <div className="mt-1 flex flex-wrap items-start justify-between gap-3">
        <div className="min-w-0">
          <h2 className="text-xl font-semibold text-zinc-950">{step.title}</h2>
          {step.detail ? <p className="mt-0.5 text-sm text-zinc-500">{step.detail}</p> : null}
        </div>
        ...
        {isTimer && timerRunning ? (
          <div className="flex shrink-0 items-center gap-2">
            ...
            <button
              ...
              className="inline-flex h-9 w-9 items-center justify-center rounded-md text-zinc-400 transition hover:bg-zinc-100 hover:text-zinc-700 disabled:opacity-50"
            >
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
        {cursor.next ? (
          <p className="text-xs text-zinc-500">
            Следом: <span className="text-zinc-700">{cursor.next.title}</span>
          </p>
        ) : <span />}
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-zinc-100 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-2">
        ...
        <span className="text-xs tabular-nums text-zinc-500">{cursor.doneCount} / {cursor.total}</span>
      </div>
