...
/**
 * Чистая сводка журнала замеров. OG = самый ранний замер, FG = самый поздний (при
 * ≥2 замерах). ABV и кажущееся сбраживание считаются из OG/FG через brewing-core;
 * при бессмысленных данных (один замер, fg ≥ og, og ≤ 1) — null. Без БД, тестируема.
 */
export const summarizeBrewMeasurements = (
  ...
  const fg = sorted.length > 1 ? sorted[sorted.length - 1]!.gravitySg : null;
