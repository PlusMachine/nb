...
const PRIORITY_STYLE: Record<FaultPriority, { box: string; chip: string; label: string }> = {
  critical: { box: "border-destructive-border bg-destructive-subtle", chip: "bg-destructive text-destructive-foreground", label: "критично" },
  high: { box: "border-warning/30 bg-warning-subtle", chip: "bg-warning text-warning-foreground", label: "важно" },
  medium: { box: "border-warning/30 bg-warning-subtle", chip: "bg-warning text-warning-foreground", label: "внимание" },
};
...
export function AlarmsPanel({ faultMask, hasDevice, onClear, clearDisabled }: Props) {
        ...
        <ul className="mt-2 space-y-1 text-xs text-muted-foreground">
          {log.map((e, i) => (
            <li key={`${e.fault}-${e.at}-${i}`} className="flex items-center gap-2">
              <span className="tabular-nums text-muted-foreground">{fmtTime(e.at)}</span>
              <span className={e.kind === "raised" ? "text-destructive" : "text-success"}>
                {e.kind === "raised" ? "возникла" : "снята"}
              </span>
              <span className="font-medium text-foreground">{FAULT_META[e.fault].title}</span>
            </li>
          ))}
        </ul>
      </details>
  ...
  if (!hasActive) {
    return <section className="rounded-xl border border-border bg-card px-4 py-2 shadow-sm">{journal}</section>;
  }
    ...
    <section className={`rounded-2xl border-2 p-5 shadow-sm ${PRIORITY_STYLE[FAULT_META[active[0]].priority].box}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="inline-flex items-center gap-2 text-sm font-semibold text-foreground">
          ...
          Аварии
          <span className="rounded-full bg-destructive px-2 py-0.5 text-xs font-semibold text-destructive-foreground">{active.length}</span>
        </p>
      ...
      <ul className="mt-3 space-y-2">
        {active.map((f) => {
          ...
          return (
            <li key={f} className={`rounded-xl border bg-card/70 p-3 ${isAcked ? "opacity-70" : ""}`}>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                ...
                {meta.cause} <span className="font-medium text-foreground">{meta.action}</span>
              </p>
            </li>
          );
        })}
