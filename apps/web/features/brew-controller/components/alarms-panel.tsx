...
import { AlertOctagon, AlertTriangle, Bell, Check } from "lucide-react";
...
export function AlarmsPanel({ faultMask, hasDevice, onClear, clearDisabled }: Props) {
  ...
  return (
    <section className={`rounded-2xl border-2 p-5 shadow-sm ${hasActive ? PRIORITY_STYLE[FAULT_META[active[0]].priority].box : "border-zinc-200 bg-white"}`}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="inline-flex items-center gap-2 text-sm font-semibold text-zinc-900">
          {hasActive ? <AlertOctagon className="h-4 w-4 text-red-600" aria-hidden /> : <Bell className="h-4 w-4 text-zinc-400" aria-hidden />}
          Аварии
          {hasActive ? (
            <span className="rounded-full bg-red-600 px-2 py-0.5 text-xs font-semibold text-white">{active.length}</span>
          ) : (
            <span className="text-xs font-normal text-zinc-500">нет активных</span>
          )}
        </p>
        {hasActive ? (
          <button
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {isAcked ? (
                    ...
                    <button
                      ...
                      className="ml-auto rounded-md border border-zinc-200 px-2 py-0.5 text-xs font-medium text-zinc-600 hover:bg-zinc-50"
                    >
                      Подтвердить
                    </button>
                  )}
