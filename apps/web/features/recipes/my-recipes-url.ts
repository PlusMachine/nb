 ...
 * По образцу {@link parseRecipesView}/{@link mergeRecipeQuery} витрины `/recipes`
 * (`recipes-url.ts`), но проще: галерея грузит рецепты целиком и фильтрует их
 * локально, поэтому URL/cookie здесь — только персистентность выбора между
 * заходами и шэрность ссылки, а не источник данных для SQL.
 *
 * - Вид (grid/list) — в cookie {@link MY_RECIPES_VIEW_COOKIE}, как у витрины.
