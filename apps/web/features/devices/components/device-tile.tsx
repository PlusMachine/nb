  ...
  return (
    <div
      className={`flex flex-col rounded-2xl border bg-white p-4 shadow-sm transition ${
        alarms.count > 0 ? "border-red-300" : "border-zinc-200"
      }`}
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0 space-y-1">
          <div className="flex items-center gap-2">
            <h3 className="truncate text-base font-semibold text-zinc-950">{tile.name}</h3>
            {tile.isDemo ? (
              <span className="inline-flex shrink-0 items-center rounded bg-indigo-100 px-1.5 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-indigo-700">
                Демо
              </span>
            ) : null}
          </div>
      ...
      {hasData ? (
        ...
        <p className="mt-3 text-sm text-zinc-500">Нет телеметрии — откройте пульт.</p>
      )}
