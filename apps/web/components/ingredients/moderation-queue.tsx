    ...
    <div className="space-y-3">
      {items.map((item) => (
        <article key={item.id} className="rounded border p-3">
          ...
          <pre className="mt-2 overflow-auto rounded bg-muted p-2 text-xs">{JSON.stringify(item.sourcePayload, null, 2)}</pre>
          <textarea placeholder="Комментарий модератора" className="mt-2 h-20 w-full rounded border p-2 text-sm" onChange={(e) => setNotes((s) => ({ ...s, [item.id]: e.target.value }))} />
          <div className="mt-2">
            <IngredientPicker
              includeCustom={false}
          ...
          <div className="mt-2 flex gap-2">
            <button className="rounded border px-2 py-1 text-sm" onClick={() => void act(item.id, "approve")}>Одобрить</button>
            <button className="rounded border px-2 py-1 text-sm" onClick={() => void act(item.id, "reject")}>Отклонить</button>
            <button className="rounded border px-2 py-1 text-sm" onClick={() => void act(item.id, "merge")}>Объединить</button>
          </div>
        </article>
      ))}
    </div>
