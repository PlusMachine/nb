...
export function RecipeSourceAttribution({
  ...
  return (
    <div className="rounded-xl border border-amber-100 bg-amber-50/60 px-3 py-2 text-xs text-zinc-600">
      <p>
        Источник:{" "}
        {showLink ? (
          <a
          ...
          <span className="font-medium text-zinc-900">{linkLabel}</span>
        )}
        {source.author ? <>, автор {source.author}</> : null}
      </p>
      {source.origin ? <p className="mt-1 leading-relaxed text-zinc-500">{source.origin}</p> : null}
    </div>
  );
}
