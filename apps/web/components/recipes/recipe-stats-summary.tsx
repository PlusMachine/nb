  ...
  const items = [
    {
      ...
      value: formatGravity(recipe.og, preferredGravityUnit),
      secondaryValue: formatGravitySecondary(recipe.og, preferredGravityUnit),
      status: fit?.og.status ?? null
    },
    {
      key: "FG",
      label: gravityLabels.fg,
      value: formatGravity(recipe.fg, preferredGravityUnit),
      secondaryValue: formatGravitySecondary(recipe.fg, preferredGravityUnit),
      ...
      <dl className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-5">
        {items.map((stat) => {
          ...
          return (
            <div
              key={stat.key}
              className="min-w-0 rounded-xl bg-muted p-3"
            >
              ...
              {isColor && colorInfo && typeof stat.value === "object" && stat.value !== null ? (
                <dd className="mt-1.5 flex min-w-0 items-center gap-1.5">
                  <BeerGlassIcon color={colorInfo.hex} size={24} className="shrink-0 text-muted-foreground" />
                  <div className="min-w-0">
                    <div className="text-sm font-semibold tabular-nums text-foreground">
                      ...
                      <span className="text-xs font-medium text-muted-foreground">EBC</span>
                    </div>
                ...
                <dd className="mt-1.5">
                  ...
                  {"secondaryValue" in stat && stat.secondaryValue ? (
                    <div className="text-xs tabular-nums text-muted-foreground">{stat.secondaryValue}</div>
                  ) : null}
                  {"sourceLabel" in stat && stat.sourceLabel ? (
                    <div className="mt-1 text-[11px] font-medium text-muted-foreground">{stat.sourceLabel}</div>
                  ) : null}
                  {"helperText" in stat && stat.helperText ? (
                    <div className="mt-1 text-[11px] text-muted-foreground">{stat.helperText}</div>
                  ) : null}
                </dd>
              )}
              {stat.status ? (
