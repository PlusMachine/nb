...
export function ArticleCard({
  ...
  return (
    <Link
      ...
      className="group block overflow-hidden rounded-[2rem] border border-zinc-200 bg-white shadow-[0_32px_90px_-62px_rgba(15,23,42,0.45)] transition duration-300 hover:-translate-y-1 hover:border-zinc-300"
      aria-label={`Открыть стиль ${article.bjcpId} ${article.title}`}
    >
      <article>
        <div className={`relative aspect-[16/10] overflow-hidden border-b border-zinc-200 ${article.heroImageUrl ? "" : mediaTheme}`} style={mediaStyle}>
        ...
        <div className="flex h-full flex-col p-6">
          ...
          <p className="mt-4 text-pretty text-[0.98rem] leading-7 text-zinc-600">{article.description}</p>

          <div className="mt-5 flex flex-wrap gap-2">
            {article.stats.slice(0, 4).map((stat) => (
              <span
                key={stat.label}
                className="inline-flex items-center gap-2 rounded-full border border-zinc-200 bg-slate-50 px-3 py-1.5 text-sm font-medium text-zinc-700"
              >
