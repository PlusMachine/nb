...
export function RichTextEditor() {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "min-h-[18rem] rounded-[1.5rem] border border-zinc-200 bg-white px-5 py-4 text-[1rem] leading-8 text-zinc-800 focus:outline-none"
      }
    },
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_24rem]">
      <section className="space-y-4 rounded-[2rem] border border-zinc-200 bg-zinc-50/80 p-5">
      ...
      <section className="rounded-[2rem] border border-zinc-200 bg-white p-5 shadow-sm">
        <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-zinc-400">JSON body</p>
        <pre className="mt-4 max-h-[34rem] overflow-auto rounded-[1.25rem] bg-zinc-950 p-4 text-xs leading-6 text-zinc-100">
          {json}
        </pre>
      </section>
    </div>
